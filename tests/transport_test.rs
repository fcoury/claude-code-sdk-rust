use claude_code_sdk::transport::{PromptInput, SubprocessCliTransport};
use claude_code_sdk::types::{ClaudeCodeOptions, McpServerConfig, PermissionMode};
...
fn test_cli_discovery() {
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                error_msg.contains("Claude Code CLI not found") || error_msg.contains("Node.js")
            );
...
fn test_command_building() {
    ...
    match transport_result {
        ...
        Err(e) => {
            println!(
                "Transport creation failed (expected if CLI not found): {e}"
            );
        }
    }
}
...
fn test_streaming_vs_string_mode() {
    ...
    let stream_prompt = PromptInput::Stream(Box::pin(tokio_stream::iter(vec![
