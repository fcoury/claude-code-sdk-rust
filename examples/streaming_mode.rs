//! Streaming mode example for the Claude Code SDK.
//!
//! This example demonstrates interactive usage with bidirectional communication.
//! It shows how to maintain a persistent connection for multiple exchanges.

use claude_code_sdk::{
    query, ClaudeCodeOptions, ContentBlock, Message, PermissionMode,
};
use std::path::PathBuf;
use tokio_stream::StreamExt;

#[tokio::main]
async fn main() -> claude_code_sdk::Result<()> {
    println!("Claude Code SDK - Streaming Mode Example");
    println!("========================================");

    // Create options for queries
    let options = ClaudeCodeOptions::builder()
        .system_prompt("You are a helpful coding assistant specializing in Rust programming.")
        .permission_mode(PermissionMode::Default)
        .allowed_tools(vec!["file_editor".to_string(), "bash".to_string()])
        .cwd(PathBuf::from("."))
        .build();

    println!("🔌 Ready to send queries to Claude...");

    // First exchange: Ask for a Rust function
    println!("📤 Sending first message...");
    let stream1 = query("Hello! Can you help me write a simple Rust function that calculates the factorial of a number?", Some(options.clone())).await;
    tokio::pin!(stream1);

    println!("📥 Receiving response...\n");
    while let Some(message) = stream1.next().await {
        match message? {
            Message::Assistant(msg) => {
                println!("🤖 Claude:");
                for content_block in &msg.content {
                    match content_block {
                        ContentBlock::Text(text_block) => {
                            println!("{}", text_block.text);
                        }
                        ContentBlock::ToolUse(tool_block) => {
                            println!(
                                "🔧 Using tool: {} (id: {})",
                                tool_block.name, tool_block.id
                            );
                        }
                        ContentBlock::ToolResult(result_block) => {
                            println!("📋 Tool result: {:?}", result_block.content);
                        }
                    }
                }
                println!();
            }
            Message::Result(result) => {
                println!("✅ First exchange completed in {}ms\n", result.duration_ms);
                break;
            }
            Message::System(sys) => {
                println!("🔧 System: {}", sys.subtype);
            }
            _ => {}
        }
    }

    // Second exchange: Ask for optimization
    println!("📤 Sending follow-up message...");
    let stream2 = query("Great! Now can you make it more efficient and add error handling for edge cases?", Some(options.clone())).await;
    tokio::pin!(stream2);

    println!("📥 Receiving follow-up response...\n");
    while let Some(message) = stream2.next().await {
        match message? {
            Message::Assistant(msg) => {
                println!("🤖 Claude:");
                for content_block in &msg.content {
                    match content_block {
                        ContentBlock::Text(text_block) => {
                            println!("{}", text_block.text);
                        }
                        ContentBlock::ToolUse(tool_block) => {
                            println!(
                                "🔧 Using tool: {} (id: {})",
                                tool_block.name, tool_block.id
                            );
                        }
                        ContentBlock::ToolResult(result_block) => {
                            println!("📋 Tool result: {:?}", result_block.content);
                        }
                    }
                }
                println!();
            }
            Message::Result(result) => {
                println!("✅ Second exchange completed in {}ms", result.duration_ms);
                println!("   Total turns in session: {}", result.num_turns);
                println!("   Session ID: {}", result.session_id);
                break;
            }
            Message::System(sys) => {
                println!("🔧 System: {}", sys.subtype);
            }
            _ => {}
        }
    }

    // Third exchange: Ask about unit tests
    println!("\n📤 Sending a third message...");
    let stream3 = query("Can you also show me how to write unit tests for the factorial function you created?", Some(options.clone())).await;
    tokio::pin!(stream3);

    println!("📥 Receiving third response...\n");
    while let Some(message) = stream3.next().await {
        match message? {
            Message::Assistant(msg) => {
                println!("🤖 Claude:");
                for content_block in &msg.content {
                    match content_block {
                        ContentBlock::Text(text_block) => {
                            println!("{}", text_block.text);
                        }
                        ContentBlock::ToolUse(tool_block) => {
                            println!(
                                "🔧 Using tool: {} (id: {})",
                                tool_block.name, tool_block.id
                            );
                        }
                        ContentBlock::ToolResult(result_block) => {
                            println!("📋 Tool result: {:?}", result_block.content);
                        }
                    }
                }
                println!();
            }
            Message::Result(result) => {
                println!("✅ Third exchange completed in {}ms", result.duration_ms);
                println!("   Total turns in session: {}", result.num_turns);
                println!("   Session ID: {}", result.session_id);
                break;
            }
            Message::System(sys) => {
                println!("🔧 System: {}", sys.subtype);
            }
            _ => {}
        }
    }

    println!("\n🎉 Streaming mode example completed successfully!");
    println!("💡 Key takeaways:");
    println!("   - Use query() function for one-shot queries with streaming responses");
    println!("   - Each query is independent but can reference previous context");
    println!("   - Process response streams message by message");
    println!("   - Stream processing allows real-time response handling");

    Ok(())
}
