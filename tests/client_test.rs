...
use claude_code_sdk::{ClaudeCodeOptions, ClaudeSDKClient, PromptInput};
...
async fn test_control_response_detection() {
    ...
    assert_eq!(
        control_response.get("type").and_then(|t| t.as_str()),
        Some("control_response")
    );
    assert_eq!(
        control_message.get("type").and_then(|t| t.as_str()),
        Some("control")
    );
    assert!(control_message.get("request_id").is_some());
    assert_eq!(
        regular_message.get("type").and_then(|t| t.as_str()),
        Some("user")
    );
}
...
async fn test_stream_timeout_handling() {
    ...
    let slow_stream = stream::iter(vec![serde_json::json!({"message": "fast"})]).chain(
        stream::iter(vec![serde_json::json!({"message": "slow"})]).then(|item| async move {
...
async fn test_client_drop_behavior() {
    ...
    // Test with options
    {
        let options = ClaudeCodeOptions::builder().system_prompt("Test").build();
...
async fn test_stream_memory_efficiency() {
    ...
    let large_stream = stream::iter(
        (0..1000).map(|i| serde_json::json!({"index": i, "data": format!("item_{}", i)})),
    );
...
async fn test_concurrent_stream_processing() {
    ...
    let handles: Vec<_> = (0..5)
        .map(|stream_id| {
            ...
            tokio::spawn(async move {
                let test_stream = stream::iter(
                    (0..10).map(move |i| serde_json::json!({"stream": stream_id, "item": i})),
                );
