use claude_code_sdk::{
    ...
    transport::{PromptInput, SubprocessCliTransport},
...
async fn collect_messages_with_errors(
    transport: &mut SubprocessCliTransport,
) -> (Vec<Message>, Vec<SdkError>) {
    let message_stream = transport.receive_messages().await;
    tokio::pin!(message_stream);
    let mut messages = Vec::new();
...
fn create_mock_cli_command(test_mode: &str, additional_args: Vec<String>) -> PathBuf {
    ...
    let timestamp = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    wrapper_path.push(format!(
        "test_cli_wrapper_{}_{}_{}.js",
        test_mode,
        std::process::id(),
        timestamp
    ));
...
async fn test_transport_connect_disconnect() {
    ...
    let mut transport = SubprocessCliTransport::new(prompt, options, Some(mock_cli_path), true)
    ...
    assert!(
        ...
        "Failed to connect: {connect_result:?}"
    );
    ...
    assert!(
        ...
        "Failed to disconnect: {disconnect_result:?}"
    );
}
...
async fn test_transport_normal_message_flow() {
    ...
    // Check message types
    assert!(
        matches!(messages[0], Message::System(_)),
        "First message should be System"
    );
    assert!(
        matches!(messages[1], Message::Assistant(_)),
        "Second message should be Assistant"
    );
    assert!(
        matches!(messages.last(), Some(Message::Result(_))),
...
async fn test_split_json_buffering() {
    ...
    assert!(
        messages.len() >= 2,
        "Should have received messages despite JSON splitting"
    );

    // Verify the long message was parsed correctly
    if let Some(Message::Assistant(assistant_msg)) =
        messages.iter().find(|m| matches!(m, Message::Assistant(_)))
    {
        if let Some(claude_code_sdk::types::ContentBlock::Text(text_block)) = assistant_msg.content.first() {
                assert!(
                    text_block.text.len() > 1000,
                    "Should have received the long text message"
                );
                assert!(
                    text_block.text.contains("very long message"),
...
async fn test_concatenated_json_buffering() {
    ...
    assert!(
        messages.len() >= 4,
        "Should have received all concatenated messages, got {}",
        messages.len()
    );
    ...
    let result_count = messages
        .iter()
        .filter(|m| matches!(m, Message::Result(_)))
        .count();
...
async fn test_large_message_handling() {
    ...
    assert!(
        !messages.is_empty(),
        "Should have received messages, got {}",
        messages.len()
    );
...
async fn test_process_error_handling() {
    ...
    assert!(
        messages.len() >= 2,
        "Should have received messages despite stderr"
    );
    ...
    assert!(
        ...
        "Should have received assistant message"
    );
...
async fn test_malformed_json_handling() {
    ...
    let json_or_stream_errors = errors
        .iter()
        .filter(|e| matches!(e, SdkError::JsonDecode(_)) || matches!(e, SdkError::Stream { .. }))
        .count();
    assert!(
        json_or_stream_errors > 0,
        "Should have encountered JSON decode or stream errors, got {} total errors",
        errors.len()
    );
...
async fn test_interactive_mode() {
    ...
    transport
        .send_request(vec![test_message], std::collections::HashMap::new())
        .await
                    ...
                    match message_result {
                        Ok(json_value) => {
                            let message = parse_message(json_value)
                                .expect("Failed to parse interactive message");
                            messages.push(message);

                            if matches!(messages.last(), Some(Message::Result(_))) {
                                response_count += 1;
                                if response_count >= 2 {
                                    // System start + our response
                                    break;
                                }
                            }
                        }
    ...
    assert!(
        !messages.is_empty(),
        "Should have received at least system start message, got {}",
        messages.len()
    );

    // Check that we got a system start message
    let has_system_start = messages.iter().any(|msg| matches!(msg, Message::System(_)));
    assert!(
        has_system_start,
        "Should have received system start message"
    );
}
...
async fn test_unicode_content_handling() {
        ...
        messages.iter().find(|m| matches!(m, Message::Assistant(_)))
    {
        ...
        assert!(
            has_unicode_text,
            "Should have received unicode text content"
        );
        ...
        assert!(
            has_unicode_tool_use,
            "Should have received unicode tool use"
        );
    }
}
...
async fn test_multiple_connect_disconnect_cycles() {
    ...
    for i in 0..3 {
        ...
        let mut transport =
            SubprocessCliTransport::new(prompt, options.clone(), Some(mock_cli_path), true)
                .expect("Failed to create transport");
        ...
        // Receive at least one message
        {
            ...
            if let Some(message_result) = message_stream.next().await {
                assert!(
                    ...
                    "Should receive valid message on cycle {i}"
                );
            }
        }
        ...
        transport
            .disconnect()
            .await
            .unwrap_or_else(|_| panic!("Failed to disconnect on cycle {i}"));
    }
}
...
async fn test_transport_drop_cleanup() {
    ...
    // Drop cleanup completed successfully
}
