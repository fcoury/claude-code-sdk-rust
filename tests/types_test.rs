...
fn test_mcp_server_stdio_full() {
    ...
    let config = McpServerConfig::Stdio {
        ...
        args: Some(vec!["-m".to_string(), "my_mcp_server".to_string(), "--verbose".to_string()]),
        env: Some(env),
    };
...
fn test_builder_pattern_comprehensive() {
    ...
    mcp_servers.insert("server1".to_string(), McpServerConfig::Stdio {
        command: "python".to_string(),
    ...
    let options = ClaudeCodeOptions::builder()
        ...
        .mcp_server("server1".to_string(), McpServerConfig::Stdio {
            command: "python".to_string(),
            args: Some(vec!["-m".to_string(), "server1".to_string()]),
            env: None,
        })
    ...
    assert_eq!(options.append_system_prompt, Some("Be concise and accurate".to_string()));
    ...
    assert_eq!(options.continue_conversation, true);
    ...
    assert_eq!(options.permission_prompt_tool_name, Some("permission_tool".to_string()));
...
fn test_builder_pattern_defaults() {
    ...
    assert_eq!(options.continue_conversation, false);
...
fn test_nested_json_in_tool_input() {
    ...
    nested_data.insert("level1".to_string(), serde_json::json!({
        "level2": {
